...
fn with_single_source() -> Result<String, UniversalError<io::Error>> {
    fs::read_to_string("./missing-single-source.txt").context("reading single source example")
}

fn with_multiple_sources(path: &std::path::Path) -> Result<i32, AppError> {
